  ...
  <div class="row">
    <div class="col-md-12">
       ...
       <div class="mb-3">
          ...
          <input type="number" name="amount" class="form-control" step="0.01" min="0" value="{{ old('amount', $expense->amount) }}">
       </div>
       ...
       <div class="mb-3">
          ...
          <select name="payment_method" id="payment_method" class="form-control">
             @foreach($paymentMethods as $paymentMethod)
              <option value="{{ $paymentMethod }}" {{ $expense->payment_method == $paymentMethod ? 'selected' : '' }}>
                {{ $paymentMethod }}
              </option>
             @endforeach
          </select>
